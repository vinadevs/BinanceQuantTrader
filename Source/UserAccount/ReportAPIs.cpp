...
#include "../KernelTrading/tools.h"
#include "../KernelTrading/pairslist.h"
#include "../KernelTrading/iofmt.h"
#include "../KernelTrading/dtf.h"
    ...
    ,const std::function<void(const rest::order_info_t &)> &tick)
{
    ...
    for ( auto &oit: trades ) {
        ...
        res.emplace_back(std::move(cpair));
    }
    ...
    if ( side ) {
        ...
        for ( auto &pit: orders.orders ) {
            for ( auto &it: pit.second ) {
                if ( it.side == side ) {
                    tmp.orders[it.symbol].emplace_back(std::move(it));
                }
            }
        }
    ...
    ,const std::function<void(const std::string &)> &tick)
{
    ...
    for ( auto &it: orders.orders ) {
        ...
        for ( auto &pit: it.second ) {
            item.order_price.emplace_back(std::move(pit.price));
        }
