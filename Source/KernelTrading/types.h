...
namespace binapi {
    ...
    namespace ws {
        ...
        struct DLL_CLASS future_trade_t {
            std::size_t E; // Event time
            std::string s; // Symbol
            std::size_t a; // Aggregated trade ID
            double_type p; // Price
            double_type q; // Quantity
            std::size_t f; // First trade ID in the aggregation
            std::size_t l; // Last trade ID in the aggregation
            std::size_t T; // Trade time
            bool m;        // Is buyer the market maker?

            static future_trade_t construct(const flatjson::fjson& json);
            friend DLL_CLASS std::ostream& operator<<(std::ostream& os, const future_trade_t& o);
        };

        /*************************************************************************************************/

        // https://developers.binance.com/docs/derivatives/usds-margined-futures/websocket-market-streams#partial-book-depth-streams
        struct DLL_CLASS future_book_t {
            std::size_t E; // Event time
            std::size_t T; // Transaction time
            std::string s; // Symbol
            std::size_t U; // First update ID
            std::size_t u; // Final update ID
            ...
            std::vector<level_t> b; // bid book
            std::vector<level_t> a; // ask book

            static future_book_t construct(const flatjson::fjson& json);
            friend DLL_CLASS std::ostream& operator<<(std::ostream& os, const future_book_t& o);
        };

        /*************************************************************************************************/

        // Binance USDⓈ-M Futures Kline Stream
        // https://binance-docs.github.io/apidocs/futures/en/#kline-candlestick-streams
        // Stream name: <symbol>@kline_<interval>
        // Example: btcusdt@kline_1m

        struct DLL_CLASS future_kline_t {

            std::size_t E;      // Event time (ms)
            std::string s;      // Symbol

            // --- inside "k" object ---

            std::size_t t;      // Kline start time (ms)
            std::size_t T;      // Kline close time (ms)
            ...
            double_type v;      // Base asset volume (contracts)

            std::size_t n;      // Number of trades
            bool x;             // Is this kline closed? (true when candle finished)

            double_type q;      // Quote asset volume
            double_type V;      // Taker buy base volume
            double_type Q;      // Taker buy quote volume

            static future_kline_t construct(const flatjson::fjson& json);
            friend DLL_CLASS std::ostream& operator<<(std::ostream& os, const future_kline_t& o);
        };

        /*************************************************************************************************/

        // Binance USDⓈ-M Futures 24hr Ticker
        // https://binance-docs.github.io/apidocs/futures/en/#24hr-ticker-price-change-statistics-streams
        // Stream: <symbol>@ticker

        struct DLL_CLASS future_ticker_t {

            std::size_t E;      // Event time (ms)
            std::string s;      // Symbol

            double_type p;      // Price change (absolute)
            double_type P;      // Price change percent (%)

            double_type c;      // Last traded price
            double_type h;      // 24h high
            double_type l;      // 24h low

            double_type v;      // 24h base volume (contracts)
            double_type q;      // 24h quote volume (USDT value)

            static future_ticker_t construct(const flatjson::fjson& json);
            friend DLL_CLASS std::ostream& operator<<(std::ostream& os, const future_ticker_t& o);
        };
        ...
        struct DLL_CLASS future_mark_price_t {

            std::size_t E;      // Event time (ms)
            std::string s;      // Symbol

            double_type p;      // Mark price (used for PnL + liquidation)
            double_type i;      // Index price (spot index)

            double_type P;      // Estimated settle price
            double_type r;      // Funding rate (next funding)

            std::size_t T;      // Next funding time (ms)

            static future_mark_price_t construct(const flatjson::fjson& json);
            friend DLL_CLASS std::ostream& operator<<(std::ostream& os, const future_mark_price_t& o);
        };

        /*************************************************************************************************/

        // Binance USDⓈ-M Futures Funding Rate
        // https://binance-docs.github.io/apidocs/futures/en/#funding-rate-stream
        // Stream: <symbol>@fundingRate

        struct DLL_CLASS future_funding_rate_t {

            std::size_t E;      // Event time (ms)
            std::string s;      // Symbol

            double_type r;      // Funding rate (e.g. 0.0001 = 0.01%)
            std::size_t T;      // Funding time (settlement timestamp)

            static future_funding_rate_t construct(const flatjson::fjson& json);
            friend DLL_CLASS std::ostream& operator<<(std::ostream& os, const future_funding_rate_t& o);
        };

        /*************************************************************************************************/

        // Binance USDⓈ-M Futures Liquidation / Force Order Stream
        // https://binance-docs.github.io/apidocs/futures/en/#liquidation-order-streams
        // Stream: <symbol>@forceOrder

        struct DLL_CLASS future_liquidation_t {
            ...
            double_type p;      // Liquidation price
            double_type q;      // Liquidation quantity

            std::size_t T;      // Trade time (ms)

            static future_liquidation_t construct(const flatjson::fjson& json);
            friend DLL_CLASS std::ostream& operator<<(std::ostream& os, const future_liquidation_t& o);
        };
