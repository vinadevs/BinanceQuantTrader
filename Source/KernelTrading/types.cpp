...
account_info_t account_info_t::construct(
    ...
    res.takerCommission = takerCommission;
    res.buyerCommission = buyerCommission;
    res.sellerCommission = sellerCommission;
    res.canTrade = canTrade;
    res.canWithdraw = canWithdraw;
    res.canDeposit = canDeposit;
...
exchange_info_t exchange_info_t::construct(const flatjson::fjson &json) {
    ...
    for ( auto idx = 0u; idx < filters.size(); ++idx ) {
        ...
        res.exchangeFilters.emplace_back(std::move(it));
    }
    ...
    for ( auto idx = 0u; idx < symbols.size(); ++idx ) {
        ...
        for ( auto idx2 = 0u; idx2 < types.size(); ++idx2 ) {
            ...
            sym.orderTypes.emplace_back(std::move(oit));
        }
        ...
        for ( auto idx2 = 0u; idx2 < filters2.size(); ++idx2 ) {
            ...
            sym.filters.emplace_back(std::move(filter));
        }

        // missing from file data exinfo.json
...
depths_t depths_t::construct(const flatjson::fjson &json) {
    ...
    for ( auto idx = 0u; idx < bids.size(); ++idx ) {
        ...
        res.bids.emplace_back(std::move(item));
    }
    ...
    for ( auto idx = 0u; idx < asks.size(); ++idx ) {
        ...
        res.asks.emplace_back(std::move(item));
    }
...
trades_t trades_t::construct(const flatjson::fjson &json) {
    ...
    for ( auto idx = 0u; idx < json.size(); ++idx ) {
        ...
        res.trades.emplace_back(std::move(item));
    }
...
klines_t klines_t::construct(const flatjson::fjson &json) {
    ...
    for ( auto idx = 0u; idx < json.size(); ++idx ) {
        ...
        res.klines.emplace_back(std::move(item));
    }
...
orders_info_t orders_info_t::construct(const flatjson::fjson &json) {
    ...
    for ( auto idx = 0u; idx < json.size(); ++idx ) {
        ...
        res.orders[item.symbol].emplace_back(std::move(item));
    }
...
new_order_info_full_t new_order_info_full_t::construct(const flatjson::fjson &json) {
    ...
    for ( auto idx = 0u; idx < fills.size(); ++idx ) {
        ...
        res.fills.emplace_back(std::move(item));
    }
...
new_order_resp_type new_order_resp_type::construct(const flatjson::fjson &json) {
    ...
    if ( json.contains("fills") ) {
        ...
        return res;
    } else if ( json.contains("standardCommissionForOrder") ) {
        // TEST
...
std::ostream &operator<<(std::ostream &os, const new_order_resp_type &o) {
    if ( const auto *p = boost::get<new_order_info_ack_t>(&o) ) {
        ...
        return os << *p;
    } else if ( const auto* p = boost::get<new_test_order_info_t>(&o) ) {
        return os << *p;
    }
...
new_test_order_info_t new_test_order_info_t::construct(const flatjson::fjson &json) {
    ...
    const auto standardCommissionForOrder_js = json.at("standardCommissionForOrder");
    __BINAPI_GET2(res.standardCommissionForOrder, maker, standardCommissionForOrder_js);
    __BINAPI_GET2(res.standardCommissionForOrder, taker, standardCommissionForOrder_js);
    const auto taxCommissionForOrder_js = json.at("taxCommissionForOrder");
    __BINAPI_GET2(res.taxCommissionForOrder, maker, taxCommissionForOrder_js);
    __BINAPI_GET2(res.taxCommissionForOrder, taker, taxCommissionForOrder_js);
    const auto discount_js = json.at("discount");
    __BINAPI_GET2(res.discount, discountAsset, discount_js);
    __BINAPI_GET2(res.discount, discount, discount_js);
    __BINAPI_GET2(res.discount, enabledForAccount, discount_js);
    __BINAPI_GET2(res.discount, enabledForSymbol, discount_js);
...
std::ostream &operator<<(std::ostream &os, const new_test_order_info_t &o) {
    os
    << "{"
    << "\"standardCommissionForOrder\":{\"maker\":" << o.standardCommissionForOrder.maker << "\","
        << "\"taker\":" << o.standardCommissionForOrder.taker << "\"," << "},"
    << "\"taxCommissionForOrder\":{\"maker\":" << o.taxCommissionForOrder.maker << "\","
        << "\"taker\":" << o.taxCommissionForOrder.taker << "\"," << "},"
    << "\"discount\":{\"discountAsset\":" << o.discount.discountAsset << "\","
        << "\"discount\":" << o.discount.discount << "\"," << "},"
        << "\"enabledForAccount\":" << o.discount.enabledForAccount << "\"," << "},"
        << "\"enabledForSymbol\":" << o.discount.enabledForSymbol << "\"," << "},"
    << "}";
...
trade_t trade_t::construct(const flatjson::fjson &json) {
    ...
    __BINAPI_GET(p);
    __BINAPI_GET(q);
    //__BINAPI_GET(b); decommision already
    //__BINAPI_GET(a); decommision already
    __BINAPI_GET(T);
    __BINAPI_GET(m);
    __BINAPI_GET(M);
...
std::ostream &operator<<(std::ostream &os, const trade_t &o) {
    ...
    << "\"p\":\"" << o.p << "\","
    << "\"q\":\"" << o.q << "\","
    //<< "\"b\":" << o.b << "," decommision already
    //<< "\"a\":" << o.a << "," decommision already
    << "\"T\":" << o.T << ","
    << "\"m\":" << (o.m ? "true" : "false") << ","
    << "\"M\":" << (o.M ? "true" : "false")
    << "}";
